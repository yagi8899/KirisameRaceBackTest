...
- 📈 **ROI分析グラフ**: 競馬場・距離・馬場・年度・月別・オッズ帯別の回収率を視覚化
- 🎯 **的中率分析**: 競馬場・距離・馬場・年度・月別の的中率を比較
...
**課題**: フィルタパネルのオッズ範囲指定が、複勝・ワイド・馬連などの戦略で正確に機能しない場合がある

**原因**:
- データには的中した馬券のオッズのみが記録されている
- 外れた馬券（4着以下、組み合わせ外）のオッズ情報が存在しない
- 現在の実装では複勝・ワイドで最小オッズを計算しているが、外れる馬券はフィルタできない

**影響範囲**:
- ✅ **単勝**: 全馬に単勝オッズがあるため正確に動作
- ⚠️ **複勝**: 3着以内に入る馬のオッズのみ記録、4着以下はフィルタ不可
- ⚠️ **ワイド**: 的中した組み合わせのオッズのみ記録、外れ組み合わせはフィルタ不可
- ⚠️ **馬連・馬単・三連複**: 同様に的中した組み合わせのみ

**回避策**:
- オッズフィルタは**参考値**として使用
- より正確なフィルタリングには、予測スコア閾値（`scoreThreshold`）の活用を推奨
- 単勝戦略ではオッズフィルタが正確に機能

**今後の対応**:
- データ収集時に全組み合わせのオッズを記録する必要がある
- または戦略クラス内でのbet決定時にオッズチェックを強化
...
- ✅ **ROI分析グラフ追加**（競馬場・距離・馬場・年度・月別・オッズ帯別の回収率可視化）
- ✅ **的中率分析グラフに月別タブ追加**（季節性・月次傾向の分析）
- ✅ フィルタパネルのstate更新バグ修正（競馬場・馬場・距離・日付・オッズ）
