...
```
Step 1: Training（1-6月）
- Stage 1で条件特定
- Stage 2でパラメータ最適化
- 最適戦略を記録

Step 2: Validation（7-9月）
- フィルタの日付を「7/1〜9/30」に変更
- Training期間で得た最適パラメータを適用
- ROIを確認

判定:
✅ ROI > 10% → Step 3へ
❌ ROI < 0% → 戦略を見直す（オーバーフィッティング）

Step 3: Test（10-12月）
- フィルタの日付を「10/1〜12/31」に変更
- 同じパラメータを適用
- ROIを確認

最終判定:
✅ 3期間すべてでROI > 10% → 信頼できる戦略
⚠️ 2期間でプラス、1期間でマイナス → やや不安定
❌ 2期間以上でマイナス → 使えない戦略
```
...
実運用での期待値: 19.3% × 0.7 = 13.5%
（70%掛けは「未知のデータ」への安全マージン）

年間期待利益（100円×100レース投資）:
投資額: 10,000円
期待リターン: 11,350円
期待利益: +1,350円
```
